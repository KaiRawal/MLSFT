...
BASE_MODEL_PATH="${BASE_MODEL_PATH:-unsloth/gpt-oss-20b}"
TEMPLATE_MODEL_ID="${TEMPLATE_MODEL_ID:-gpt-oss-20b}"
FINETUNE_TEMPLATE_NAME="${FINETUNE_TEMPLATE_NAME:-chatml}"
...
echo "Using template model id override for eval generation: ${TEMPLATE_MODEL_ID}"
...
run_language_pipeline() {
  ...
  local repo_name="GPT-OSS-20B-${lang_code}-SynthDolly-1A"
  ...
  if run_step "${language}" "[1/5]" "Pre-finetune English eval" "${step_log}" python "${ENGLISH_EVAL_SCRIPT}" --config <(cat <<JSON
{
  ...
  "generation_model_id_for_template": "${TEMPLATE_MODEL_ID}",
